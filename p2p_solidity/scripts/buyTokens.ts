...
// const approveMUSDCTxn=   await musdcContract.connect(signer).approve(p2pContract.getAddress(), 5)
// await approveMUSDCTxn.wait();
// console.log("approveTxn", await approveMUSDCTxn.hash);

const approveSUSDCTxn=   await susdcContract.connect(signer).approve(p2pContract.getAddress(), 20)
...
// const  depositTxn=await p2pContract.connect(signer2).depositToken(200, "MUSDC");
// await depositTxn.wait();
// console.log("depositTxn", await depositTxn.hash);

// const approveSUSDCTxn=   await susdcContract.connect(signer).approve(p2pContract.getAddress(), 25);
// await approveSUSDCTxn.wait();
// console.log("approveTxn", await approveSUSDCTxn.hash);
    // console.log("Approved:", await approvTxn.hash);
// const depositTxn = await p2pContract.connect(signer2).postSellOrder(signer2.address, "SUSDC", 10, "INR", 91);
//     await depositTxn.wait();
...
const buyTxn=await p2pContract.connect(signer).processBuyOrder(signer.address,2, "SUSDC",10);
await buyTxn.wait(); 
console.log("buyTxn", await buyTxn.hash);
